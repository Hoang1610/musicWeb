...
export default function AllPlaylistsPage() {
  ...
  let categories = dataPlayList?.items.map((item: any) =>
  ...
  if (dataPlayList) {
    dataPlayList.items = dataPlayList?.items.map((item: any) => {
      const name = getCategory(item.title);
      return {
        ...item,
        category: name,
      };
  ...
  const filteredPlaylists = dataPlayList?.items?.filter((playlist: any) => {
    const matchesSearch =
      playlist.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      playlist.sortDescription.toLowerCase().includes(searchTerm.toLowerCase());
